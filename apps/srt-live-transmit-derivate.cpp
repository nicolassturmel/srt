...
int parse_args(LiveTransmitConfig &cfg, int argc, char** argv)
{
    ...
    if (params[""].size() != 3 && !print_help && !print_version)
    {
...
int main(int argc, char** argv)
{
    ...
    unique_ptr<Target> derivative;
    bool derivativeConnected = false;
    ...
    try {
        ...
        while (!int_state && !timer_state)
        {
            ...
            if (!derivative.get())
            {
                derivative = Target::Create(cfg.derivative);
                if (!derivative.get())
                {
                    cerr << "Unsupported derivative type" << endl;
                    return 1;
                }
                ...
                // OUT - to check the connection state changes
                int events = SRT_EPOLL_IN | SRT_EPOLL_OUT | SRT_EPOLL_ERR;
                switch(derivative->uri.type())
                {
                case UriParser::SRT:
                    if (srt_epoll_add_usock(pollid,
                        derivative->GetSRTSocket(), &events))
                    {
                        cerr << "Failed to add SRT destination to poll, "
                            << derivative->GetSRTSocket() << endl;
                        return 1;
                    }
                    break;
                default:
                ...
                &sysrfds[0], &sysrfdslen, 0, 0) >= 0)
            {
                ...
                while (!dataqueue.empty())
                {
                    ...
                    derivative->Write(pkt->payload.data(), pkt->payload.size(), cfg.srctime ? pkt->time : 0, out_stats);
                    dataqueue.pop_front();
                }
